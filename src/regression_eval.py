...
def do_eval(args: argparse.Namespace) -> None:
    ...
    test_sequence = vecchio.make_data.MultiLabelRegressionEBSDSequence(x_test, y_test, args.batch_size)

    mdl_factory = find_model(args.trained_model, list(args.models))(n_labels=len(args.label_columns))
...
def make_parser(_parser: typing.Optional[argparse.ArgumentParser] = None) -> argparse.ArgumentParser:
    if not _parser:
        _parser = argparse.ArgumentParser(description='Evaluate a Regression ML Model')

    models = {n: n for n in
              [*vecchio.types.RegressionModel.__subclasses__(), *vecchio.types.ClassificationModel.__subclasses__()]}
